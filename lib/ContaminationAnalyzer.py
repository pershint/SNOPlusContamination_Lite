...
class ContaminationEstimator(object):
    ...
    def _CalculateAcceptances(self):
        '''
        Use the given sacrifice summary entries to calculate the
        acceptance (1 - sacrifice) of each bifurcation branch.  
        The acceptance is taken as the mean of all variable's 
        acceptances, and the acceptance uncertainty is the 
    ...
    def SaveContaminationSummary(self,savedir,savename):
        '''
        Save a JSON summary of the contamination analysis to the
        path/filename specified
        '''
...
class LogLikelihoodContam(ContaminationEstimator):
    '''
    Estimates are found for y1, y2, and B (and their uncertainties) using a 
    negative log likelihood minimization approach.  Likelihood function associated
    with the probability the a,b,c, and d box values come from the estimates fitted.
    '''
    ...
    def nll_minuit(self,y1,y2,B):
        '''Same as nll function defined above, but Minuit takes in
        it's constants and fit parameters differently than fmin.'''
    ...
    def FitContaminationParameters(self,useMinuit=True):
        ...
        function defined.  Fitting can be done with either minuit or
        fmin from scipy.'''
    ...
    def SaveMinimizationSummary(self,savedir,savename):
        '''Save the summary of minimization execution log'''
...
class LETAContamination(ContaminationEstimator):
    ...
    def __init__(self, Bifurcation_Summary=None, Sacrifice_Summary=None):
        super(LETAContamination,self).__init__(Bifurcation_Summary=Bifurcation_Summary,
                Sacrifice_Summary=Sacrifice_Summary)
        self.setting_upperlimit = False
        self._CheckIfUpperLimit()
        self.N = self.a + self.b + self.c + self.d
        ...
        self.contamination_summary['type'] = 'LETA'


    def _CheckIfUpperLimit(self):
        '''
        Checks whether or not the contamination calculation is going
        to be an upper limit set.  If one of the boxes is zero, the contam.
        and contam. unc. would be calculated as zero.  Instead, the zero
        box is set to 1.15 (68% of the time, a value of zero would come from
        a Poisson distribution of mean less than 1.15) and a flag is set to
        confirm this can only be used to set a limit.
        '''
        if self.a == 0:
            self.a = 1.15
            self.a_unc = 0
        ...
        if self.d == 0:
            ...
            self.d_unc = 0
            self.setting_upperlimit = True
        if self.setting_upperlimit:
            print("SETTING UPPER LIMIT ON CONTAMINATION")
...
class NDContamination(ContaminationEstimator):
    ...
    number of signal events in the a-box.  An alterative approximation
    is made that all events in the b,c, and d boxes are background.  The
    issue is the equation now becomes overconstrained.'''
    ...
    def _avg_y1y2(self,a,b,c,x1,x2,bkg):
        '''
        Returns the average of the y1y2 values acquired from the 
    ...
    def _highest_y1y2(self,a,b,c,x1,x2,bkg):
        '''
        Returns the highest of the y1y2 values calculated from the
        over-constrained equations.
        '''
    ...
    def _leastsq_y1y2(self,a,b,c,x1,x2,bkg):
        '''
        Finds the best-fit y1y2 value from the over-constrained
        equations using a least squares approach.  This is described in
    ...
    def CalculateContaminationValues(self):
        '''
        Calculates all variables in the over-constrained contamination 
        equations and stores them in the self.contamination_summary
        dictionary.
        '''
        ...
        self.contamination_summary["y1*y2"] = self._y1y2(self.a,self.x1,\
                self.x2,self.bkg_events)
        self.contamination_summary["y1*y2*B"] = self._y1y2(self.a,self.x1,\
